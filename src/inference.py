...
class FakeNewsModelServer:
    ...
    def load_models(self):
        ...
        # Initialize LSTM model with vocab size
    ...
    def predict(self, text: str) -> dict:
        ...
        prob_lr = self.lr.predict_proba(X_tfidf)[0][1]
        prob_nb = self.nb.predict_proba(X_tfidf)[0][1]
        prob_rf = self.rf.predict_proba(X_tfidf)[0][1]
        ...
        with torch.no_grad():
            prob_lstm = self.lstm_model(input_tensor).item()
        ...
        with torch.no_grad():
            ...
            prob_bert = F.softmax(logits, dim=1)[0][1].item()
        ...
        model_breakdown = {
            "logistic": get_label_str(prob_lr),
            "naive_bayes": get_label_str(prob_nb),
            "random_forest": get_label_str(prob_rf),
            "lstm": get_label_str(prob_lstm),
            "distilbert": get_label_str(prob_bert)
        }
